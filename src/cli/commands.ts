...
export class CLI {
    ...
    while (running) {
      ...
      try {
        switch (choice) {
          ...
          case '2': await this.handleListTasks(); break;
          case '3': await this.handleUpdateTask(); break;
          case '4': await this.handleDeleteTask(); break;
          case '5': await this.handleSearchTasks(); break;
          case '6': await this.handleFilterTasks(); break;
          case '7': await this.handleSortTasks(); break;
          case '8': await this.handleExportJson(); break;
          case '9': await this.handleExportCsv(); break;
          case '10': await this.handleViewStatistics(); break;
  ...
  private async handleUpdateTask(): Promise<void> {
    ...
    const description = await this.question('New Description (leave blank to skip): ');
  ...
  private async handleFilterTasks(): Promise<void> {
    const priorityStr = await this.question('Priority filter (leave blank for all): ');
    const priority = priorityStr ? Priority[priorityStr.toUpperCase() as keyof typeof Priority] : undefined;
  ...
  private printTasks(tasks: any[]): void {
    ...
    console.log('---------------------------------');
    tasks.forEach(t => {
      console.log(`${t.id} | ${t.title} | ${t.priority} | ${t.dueDate.toLocaleDateString()}`);
    });
  }
}
